...
if __name__ == "__main__":
    ...
    if not filtered_stores.empty:
        ...
        total_times = filtered_stores["Total Time"]
        frequencies = filtered_stores["Frequency of Basket Clean"]

        # Generate schedule for 5 am to 9 am slot (240 minutes)
        schedule_5am_9am = create_schedule(stores, store_names, store_postcodes, total_times, slot_minutes=240)

        # Generate schedule for 5 am to 9 pm slot (960 minutes with a 30-minute break)
        schedule_5am_9pm = create_schedule(stores, store_names, store_postcodes, total_times, slot_minutes=960, break_time=30)
    ...
    if not filtered_stores.empty:
        ...
        if not nearby_stores.empty:
            ...
            m = ro.plot_store_route(nearby_stores, current_lat, current_lon)
            folium_static(m)

    df1 = pd.read_csv("Optimized_Schedule__5_AM_-_9_AM.csv")
    df1["Store"] = df1["Store"].astype(str)
    df2 = pd.read_csv("Optimized_Schedule__5_AM_-_9_PM.csv")
    df2["Store"] = df2["Store"].astype(str)

    st.subheader("Optimised Yearly schedule for 5 AM to 9 AM")
    st.dataframe(df1)
